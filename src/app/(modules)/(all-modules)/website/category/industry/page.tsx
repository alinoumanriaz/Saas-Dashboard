...
import { GET_PAGINATED_INDUSTRIES, DELETE_INDUSTRIES } from "@/graphql/current-website-queries/industry.query";
import {
  DataListPage,
  FilterConfig,
...
const Page = () => {
  ...
  const industriesData = allIndustries.map((industry) => ({
    ...industry,
    id: industry.id,
  }));
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (data?.deleteIndustries?.success) {
        ...
        toast.success(data.deleteIndustries.message, { position: "top-center" });
        setShowConfirmationModel(false);
        refetch();
        setSelectedIdsForDeletion([]);
      } else {
      ...
      toast.error(err.message || "Failed to delete industries", { position: "top-center" });
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const editHandler = (industry: any) => {
    // Use the original industry object (with _id)
    const original = allIndustries.find((i: any) => i.id === industry.id) || industry;
    setIsEditMode(true);
  ...
  return (
    <DataListPage
      ...
      subtitle={<span>Total Industries: {totalIndustries}</span>}
      // stats={stats}
      ...
      addDisabled={false}
      data={industriesData}  // <-- use the mapped data with `id`
    ...
    >
      ...
      {showConfirmationModel && (
        <ConfirmationBox
          ...
          message={`Are you sure you want to delete ${selectedIdsForDeletion.length} industr${
            selectedIdsForDeletion.length === 1 ? "y" : "ies"
