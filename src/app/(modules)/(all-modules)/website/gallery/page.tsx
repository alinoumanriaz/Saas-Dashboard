...
const Page = () => {
  ...
  const FOLDER_NAME: any = companyCurrentWebsite?.cloudinary?.folderName
  const CLOUDINARY_NAME: any = companyCurrentWebsite?.cloudinary?.cloudinaryName
  const CLOUDINARY_NAME_API_KEY: any = companyCurrentWebsite?.cloudinary?.cloudinaryNameApiKey
  const CLOUDINARY_NAME_API_KEY_SECRET: any = companyCurrentWebsite?.cloudinary?.cloudinaryNameApiKeySecret
  ...
  const fetchImages = useCallback(async (cursor: string | null = null) => {
    try {
      ...
      if (!FOLDER_NAME || !CLOUDINARY_NAME) {
      ...
      const params: any = {
        ...
        cloudinaryApiKey: CLOUDINARY_NAME_API_KEY,
        cloudinaryApiKeySecret: CLOUDINARY_NAME_API_KEY_SECRET,
        limit: 50
      };

      // Fix: Use next_cursor instead of cursor for the API
      ...
      setImages((prev) =>
        cursor ? [...prev, ...newImages] : newImages
      );
  ...
  }, [FOLDER_NAME, CLOUDINARY_NAME, CLOUDINARY_NAME_API_KEY, CLOUDINARY_NAME_API_KEY_SECRET]); // Add all dependencies

  useEffect(() => {
    // Only fetch if we have the required config
    if (activeTab === "gallery" && !hasFetchedGallery && FOLDER_NAME && CLOUDINARY_NAME) {
      setHasFetchedGallery(true);
      fetchImages();
    }
  }, [activeTab, hasFetchedGallery, fetchImages, FOLDER_NAME, CLOUDINARY_NAME]);
  ...
  const handleImageDelete = useCallback(async () => {
    ...
    try {
      ...
      await axios.delete("/api/cloudinary", {
        data: { public_ids, folder: FOLDER_NAME },
      });
