...
import GalleryModel from "@/components/popup/models/GalleryModel";
import {
  CheckCircle2,
  XCircle,
  AlertCircle,
...
type BannerKey = "mobilebannerImage" | "desktopbannerImage";
...
const FieldStatusBadge = ({
  ...
  const map: Record<
    string,
    { variant: "default" | "destructive" | "secondary"; icon: React.ReactNode; label: string }
  > = {
    checking: {
      variant: "secondary",
...
const Page = () => {
  ...
  const [bannerImages, setBannerImages] = useState<{
    ...
    mobilebannerImage: { url: "", alt: "" },
    desktopbannerImage: { url: "", alt: "" },
  });

  // Gallery mode
  const [showGalleryOpen, setShowGalleryOpen] = useState(false);
  const [galleryMode, setGalleryMode] = useState<BannerKey>("desktopbannerImage");

  // Unique / validation check states
  const [h1CheckStatus, setH1CheckStatus] = useState<CheckStatus>("idle");
  const [metaTitleCheckStatus, setMetaTitleCheckStatus] = useState<CheckStatus>("idle");
  const [metaDescCheckStatus, setMetaDescCheckStatus] = useState<LengthStatus>("idle");

  // Tracks whether the form has unsaved edits (used to confirm before cancelling)
  ...
  const [checkUnique, { loading: uniqueLoading }] = useLazyQuery<any>(CHECK_UNIQUE, {
    fetchPolicy: "network-only",
  });
  ...
  useEffect(() => {
    if (data?.getPage && !isEditing) {
      ...
      setBannerImages({
        mobilebannerImage: page.bannerImages?.mobilebannerImage || { url: "", alt: "" },
        desktopbannerImage: page.bannerImages?.desktopbannerImage || { url: "", alt: "" },
      });
    }
  }, [data, isEditing]);
  ...
  const handleSave = async () => {
    ...
    console.log("Saving page with input:", input);
  ...
  const handleSelectedImage = (images: any[]) => {
    if (images.length > 0) {
      ...
      setBannerImages((prev) => ({ ...prev, [galleryMode]: selectedImage }));
      markDirty();
    }
    setShowGalleryOpen(false);
  };

  const openGallery = (mode: BannerKey) => {
    setGalleryMode(mode);
    setShowGalleryOpen(true);
  };

  const removeImage = (mode: BannerKey) => {
    setBannerImages((prev) => ({ ...prev, [mode]: { url: "", alt: "" } }));
    markDirty();
  };

  const updateImageAlt = (mode: BannerKey, alt: string) => {
    setBannerImages((prev) => ({ ...prev, [mode]: { ...prev[mode], alt } }));
    markDirty();
  };
        ...
        <Card className="border-destructive/30">
          <CardContent className="flex flex-col items-center justify-center gap-3 py-16 text-center">
            ...
            <div className="space-y-1">
              <h3 className="text-lg font-semibold">Couldn&apos;t load the home page</h3>
              <p className="text-sm text-muted-foreground">
                Something went wrong while fetching this page. Check your connection and try again.
              </p>
            </div>
    ...
    return (
      <div className="mx-auto min-w-2xl p-6">
        <CardContent className="flex flex-col items-center justify-center gap-3 py-16 text-center">
          ...
          <div className="space-y-1">
            <h3 className="text-lg font-semibold">No home page yet</h3>
            <p className="text-sm text-muted-foreground">
              Once a home page is created, it will appear here for editing.
            </p>
          </div>
        </CardContent>
      </div>
  ...
  // At this point we are guaranteed to have page data
  const page = data.getPage;
        ...
        {!isEditing ? (
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1 text-sm font-medium text-blue-700 dark:bg-blue-950 dark:text-blue-300">
              ...
              Editing
              {isDirty && <span className="text-blue-500">· unsaved changes</span>}
            </span>
            <div className="flex gap-2">
              <Button variant="outline" onClick={handleCancel} disabled={updating}>
                Cancel
              </Button>
              <Button onClick={handleSave} disabled={updating || !tempContent.trim()} className="gap-2">
        ...
        <CardContent className="p-0">
          {isEditing ? (
              ...
              <div className="space-y-8 p-6">
                ...
                <section aria-labelledby="seo-status-heading">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <h2 id="seo-status-heading" className="text-sm font-semibold text-muted-foreground">
                      SEO readiness
                    </h2>
                  </div>
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    {[
                      ...
                      },
                      {
                        label: "Meta title",
                        ok: !!metaTitle.trim(),
                        msg: metaTitle.trim() ? `${metaTitle.length}/60 characters` : "Missing — required",
                      },
                      {
                        label: "Meta description",
                        ok: metaDescCheckStatus === "good" || metaDescStatusForCount === "good",
                        msg:
                      ...
                      },
                      {
                        ...
                        msg: shortDescription.trim() ? `${shortDescription.length}/300 characters` : "Optional",
                      },
                    ].map((item) => (
                      <div
                        key={item.label}
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">{item.msg}</p>
                      </div>
                    ))}
                  </div>
                </section>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        placeholder="e.g. Build better software, faster"
                      />
                      <TooltipProvider>
                        <Tooltip>
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            ...
                            >
                              {uniqueLoading && metaTitleCheckStatus === "checking" ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                              ) : (
                                <Search className="h-4 w-4" />
                              )}
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <CharCountBadge length={metaDescription.length} min={120} max={160} />
                    </div>
                    <Textarea
                      id="meta-desc"
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Not shown in search results — used for internal previews and listings.
                    </p>
                  </div>
                </div>
                ...
                <section className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold text-muted-foreground">Banner images</h2>
                  </div>
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    {(
                      [
                        { key: "mobilebannerImage" as const, label: "Mobile banner", ratio: "aspect-[9/16] max-h-72" },
                        { key: "desktopbannerImage" as const, label: "Desktop banner", ratio: "aspect-video" },
                      ]
                    ).map(({ key, label, ratio }) => (
                      <div key={key} className="space-y-2">
                        ...
                        <div
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === " ") openGallery(key);
                          }}
                          className={`group relative w-full ${ratio} mx-auto cursor-pointer overflow-hidden rounded-lg border-2 border-dashed bg-muted/20 transition-colors hover:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary`}
                        >
                          {bannerImages[key].url ? (
                            <>
                              <Image
                                src={bannerImages[key].url}
                                alt={bannerImages[key].alt || label}
                                fill
                              ...
                              <Button
                                ...
                                variant="destructive"
                                size="icon"
                                className="absolute right-2 top-2 h-7 w-7 rounded-full opacity-0 shadow transition-opacity group-hover:opacity-100"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  removeImage(key);
                                }}
                            ...
                            <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center text-muted-foreground">
                              ...
                              <p className="text-sm">Click to choose an image</p>
                            </div>
                          )}
                        </div>
                        <Input
                          value={bannerImages[key].alt || ""}
                          onChange={(e) => updateImageAlt(key, e.target.value)}
                          placeholder="Alt text (describes the image for accessibility & SEO)"
                          disabled={!bannerImages[key].url}
                          className="text-sm"
                        />
                      </div>
                    ))}
                ...
                <section className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold text-muted-foreground">Page content</h2>
                  </div>
                  <div className="overflow-hidden rounded-lg border">
                    <TiptopEditor initialValue={tempContent} onChange={handleContentChange} height={`h-[400px]`} />
                  </div>
                </section>
              </div>
            </ScrollArea>
            ...
            <Tabs defaultValue="content" className="w-full">
                ...
                <ScrollArea className="h-[calc(100vh-320px)] rounded-lg border">
                  {page.content ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-2 p-12 text-center text-muted-foreground">
                      ...
                      <p className="text-sm">No content yet. Click &quot;Edit content&quot; to get started.</p>
                    </div>
                  )}
                </ScrollArea>
              </TabsContent>
                  ...
                  <CardContent>
                    <div className="space-y-1 rounded-lg border bg-muted/20 p-4">
                      ...
                      <h3 className="text-xl font-medium text-blue-800 hover:underline dark:text-blue-400">
                        {page.metaTitle || "Home — Your Company"}
                      </h3>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {[
                    ...
                    },
                    {
                      ...
                      badge: <CharCountBadge length={page.metaTitle?.length || 0} max={60} />,
                    },
                    {
                    ...
                    },
                    {
                      ...
                      badge: <CharCountBadge length={page.shortDescription?.length || 0} max={300} />,
                    },
                  ].map((item) => (
                    <Card key={item.title}>
                      ...
                      <CardContent>
                        <p className={item.value ? "text-sm text-foreground" : "text-sm italic text-muted-foreground"}>
                          {item.value || "Not set"}
                        </p>
              ...
              <TabsContent value="banner" className="p-6 pt-4">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  {(["mobilebannerImage", "desktopbannerImage"] as const).map((key) => {
                    const img = page.bannerImages?.[key];
                    const label = key === "mobilebannerImage" ? "Mobile banner" : "Desktop banner";
                    return (
                      <Card key={key}>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                          <CardTitle className="text-sm font-medium">{label}</CardTitle>
                          <Badge variant={img?.url ? "default" : "secondary"} className="font-normal">
                            {img?.url ? "Set" : "Not set"}
                          </Badge>
                        </CardHeader>
                        <CardContent>
                          {img?.url ? (
                            <div className="relative aspect-video w-full overflow-hidden rounded-lg border">
                              <Image src={img.url} alt={img.alt || label} fill className="object-cover" />
                            </div>
                          ) : (
                            <div className="flex h-40 flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed text-muted-foreground">
                              <ImagePlus className="h-7 w-7" />
                          ...
                          {img?.alt && (
                            <p className="mt-2 text-sm text-muted-foreground">
                              <span className="font-medium text-foreground">Alt text:</span> {img.alt}
                            </p>
                          )}
                        </CardContent>
                      </Card>
