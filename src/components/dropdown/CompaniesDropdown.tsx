...
const CompaniesDropdown = ({
  ...
  useEffect(() => {
    if (selectedCompanyMember) {
      ...
      setSelected(data[0]);
      dispatch(setCompanyMember(data[0]));
    }
  ...
  return (
    <div className="relative w-full group select-none">
      ...
      <div
        ...
        className="text-black w-full px-2 py-3 border-b-2 border-b-gray-200 rounded cursor-pointer flex justify-between items-center transition-colors duration-200"
      >
        ...
        <div className="py-1 rounded-md  text-black/60 group-hover:bg-black/10 group-hover:text-black transition-all duration-200">
          <svg
      ...
      {/* Dropdown */}
      {open && (
        <div onMouseLeave={() => setOpen(false)} className="fixed w-96 ring-1 rounded-xl ring-black/10 bg-white mt-1 shadow-sm z-50 max-h-60 overflow-auto">
          <div className="p-3">
            {data.map((cm) => (
              <div
                key={cm.id}
              ...
              >
                <div className="flex space-x-2 justify-center items-center">
                  ...
                  <span className="text-xs text-black px-1.5 pt-0.5 rounded-md bg-black/10 transition-colors duration-200">
                    {cm.role}
                  </span>
                </div>
                <div>
          ...
          <div className="w-full hover:bg-black/5 cursor-pointer text-black border-t border-black/10 py-4">
