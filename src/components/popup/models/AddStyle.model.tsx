    ...
    try {
      ...
      if (isEditMode && selectedData?.id) {
        ...
        if (result.data?.updateStyle?.success) {
          toast.success(result.data.updateStyle.message || "Style updated successfully");
        } else {
          toast.error(result.data?.updateStyle?.message || "Failed to update style");
        }
      } else {
        result = await createStyle({ variables: { input: cleanInputData } });
        if (result.data?.createStyle?.success) {
          toast.success(result.data.createStyle.message || "Style created successfully");
        } else {
          toast.error(result.data?.createStyle?.message || "Failed to create style");
        }
      }
      if (refetch) refetch();
      onCancel();
