...
import {
  CHECK_STYLE_H1_TAG_UNIQUE,
  CHECK_STYLE_META_TITLE_UNIQUE,
  CHECK_STYLE_SLUG_UNIQUE,
  CREATE_STYLE,
  UPDATE_STYLE,
} from "@/graphql/current-website-queries/style.query";
...
type FormValues = z.infer<typeof styleSchema>;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-auto">
      ...
      <div>
        ...
        <Controller
          name="name"
          control={control}
          render={({ field }) => <Input placeholder="Enter style name" {...field} />}
        />
...
const AddStyle = ({
  ...
  const [checkSlug, { loading: checkingSlug, data: slugData, error: slugError }] =
    useLazyQuery<any>(CHECK_STYLE_SLUG_UNIQUE);
  const [checkH1Tag, { loading: checkingH1Tag, data: h1TagData, error: h1TagError }] =
    useLazyQuery<any>(CHECK_STYLE_H1_TAG_UNIQUE);
  const [checkMetaTitle, { loading: checkingMetaTitle, data: metaTitleData, error: metaTitleError }] =
    useLazyQuery<any>(CHECK_STYLE_META_TITLE_UNIQUE);

  // Update check feedback when data arrives
  useEffect(() => {
    if (slugData?.checkStyleSlugUnique) {
      const isUnique = slugData.checkStyleSlugUnique.isUnique;
  ...
  useEffect(() => {
    if (h1TagData?.checkStyleH1TagUnique) {
      const isUnique = h1TagData.checkStyleH1TagUnique.isUnique;
  ...
  useEffect(() => {
    if (metaTitleData?.checkStyleMetaTitleUnique) {
      const isUnique = metaTitleData.checkStyleMetaTitleUnique.isUnique;
  ...
  // ---------- Mutations ----------
  const [createStyle, { loading: createLoading }] = useMutation<any>(CREATE_STYLE);
  const [updateStyle, { loading: updateLoading }] = useMutation<any>(UPDATE_STYLE);
    ...
    try {
      ...
      if (isEditMode && selectedData?.id) {
        ...
        if (result.data?.updateStyle?.success) {
          toast.success(result.data.updateStyle.message || "Style updated successfully", { position: "top-center" });
        } else {
          toast.error(result.data?.updateStyle?.message || "Failed to update style", { position: "top-center" });
        }
      } else {
        result = await createStyle({ variables: { input: cleanInputData } });
        if (result.data?.createStyle?.success) {
          toast.success(result.data.createStyle.message || "Style created successfully", { position: "top-center" });
        } else {
          toast.error(result.data?.createStyle?.message || "Failed to create style", { position: "top-center" });
        }
      }
      if (refetch) refetch();
      onCancel();
  ...
  return (
    <>
      <DynamicForm<FormValues>
        ...
        title={isEditMode ? "Edit Style" : "Add New Style"}
        tabs={tabs}
        defaultValues={defaultValues}
        validationSchema={styleSchema}
        onSubmit={onSubmit}
        isSubmitting={isSubmitting}
        submitLabel={isEditMode ? "Update Style" : "Create Style"}
      />
