...
import {
  CREATE_MATERIAL,
  UPDATE_MATERIAL,
} from "../../../graphql/query/material.query";
...
const AddMaterial = ({
  ...
  const [updateMaterial, { loading: updateLoading }] =
    useMutation(UPDATE_MATERIAL);
  const [createMaterial, { loading: createLoading }] =
    ...
    try {
      if (isEditMode && selectedData?._id) {
        ...
        if (result.data?.updateMaterial?.success) {
          ...
          toast.error(
            result.data?.updateMaterial?.message || "Failed to update material"
          );
        }
      } else {
        ...
        if (result.data?.createMaterial?.success) {
          toast.success(
            result.data.createMaterial.message ||
              "Material created successfully"
          );
          onCancel();
        } else {
          toast.error(
            result.data?.createMaterial?.message || "Failed to create material"
          );
        }
      }
